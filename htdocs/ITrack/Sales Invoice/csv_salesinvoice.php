...
if(xsrf_guard())
{
    ...
    if($_POST['cancel']) 
    {
        ...
        header("location: listview_salesinvoice.php?filter_field=$filter_field_used&filter=$filter_used&page_from=$page_from");
    }
    ...
    if($_POST['submit'])
    {
        ...
        $dbh_salesinvoice = new salesinvoice;
        ...
        if($message=="")
        {
            ...
            $csv_name = '/' . $_SESSION['user'] . '_salesinvoice_' . $timestamp . '.csv';
            $filename = TMP_DIRECTORY . $csv_name;
            $linkname = TMP_HYPERLINK_PATH . $csv_name;

            $csv_contents = $dbh_salesinvoice->export_to_csv();
...
$html->draw_header('CSV Exporter: salesinvoice', $message, $message_type);
...
$html->draw_text_field('Employee', 'Employee', FALSE, 'text', TRUE);
$html->draw_text_field('SalesOrder Id', 'SalesOrder_id', FALSE, 'text', TRUE);
$html->draw_text_field('Customer', 'Customer_id', FALSE, 'text', TRUE);
